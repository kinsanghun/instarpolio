    ...
    return (
        <div className="post">
            ...
            <div className="content-box">
                ...
                {post.link.length > 0 ? <div>link : <a style={{display:"inline-block", color:"#9Dc5F7"}} href={post.link}>{post.link}</a></div> : <></>}
                {post.git.length > 0 ? <div>Github : <a style={{display:"inline-block", color:"#9DB5F7"}} href={post.git}>{post.git}</a></div> : <></>}
